...
def lambda_handler(event, context):
    ...
    provisioner = get_provisioner(httpClient, kubeApiServerEndpoint, kubeApiKey)
    ...
    if provisioner is not None:
        if provisioner['items'][0]['spec']['requirements'][0]['values'][0] == 'spot':
            completed = True
            message = "Task Completed"
            progressPercent = 100
        else:
            message = "Provisioner in not running"
    else:
        message = "Provisioner is not found"
...
def get_provisioner(httpClient, kubeApiServerEndpoint, kubeApiKey):
    res = httpClient.request('GET', kubeApiServerEndpoint + '/apis/karpenter.sh/v1alpha5/provisioners', headers={"authorization" : "Bearer " + kubeApiKey})
    if res.status == 200:
        provisioner = json.loads(res.data.decode('utf-8'))
    else:
        logging.error('Get provisioner Failed', exc_info=True)
        provisioner = None
